...
def acceptlist():
    if names in name_list :
        output_area.configure(state='disabled')
        output_area.delete(1.0, tk.END)
        output_area.insert(tk.End, 'Vállalta')
        output_area.configure(state='enabled')
        with open('RANacceptlist.txt', 'w') as list:
            list.write('RANacceptlist.txt')
    else:
        output_area.configure(state='disabled')
        output_area.delete(1.0, tk.END)
        output_area.insert(tk.END, 'Nem tudta vállalni')
        output_area.configure(state='enabled')
...
def acceptlist():
    selected_indices = name_list.curselection()
    if selected_indices:
        accepted_name = [name_list.get(i) for i in selected_indices][0]
        output_area.configure(state='normal')
        output_area.delete(1.0, tk.END)
        output_area.insert(tk.END, f'{accepted_name} vállalta.')
        output_area.configure(state='disabled')

        # Write to log file
        log_file_name = f"{accepted_name}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.txt"
        with open(log_file_name, 'w') as f:
            f.write(f"{accepted_name} vállalta ezen a napon: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    else:
        output_area.configure(state='normal')
        output_area.delete(1.0, tk.END)
        output_area.insert(tk.END, 'Nem tudta vállalni')
